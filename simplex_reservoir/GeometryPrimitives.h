...
////Point
//////////////////////////////////////////////////////////////////////////
template<int d> class Point : public ImplicitGeometry<d>
{
	Typedef_VectorDii(d);
public:
	VectorD point;
	Point(const VectorD& _p) :point(_p) {}
	virtual bool Inside(const VectorD& pos) const { return false; }
	virtual real Phi(const VectorD& pos) const
	{
...
////Capsule
//////////////////////////////////////////////////////////////////////////
template<int d> class Capsule : public ImplicitGeometry<d>
{Typedef_VectorDii(d);
public:
	VectorD center = VectorD::Zero();
	real radius = (real).1;
	...
	Capsule(const VectorD& _center, const real _radius, const VectorD& _normal, const real _height) :center(_center), radius(_radius), normal(_normal), height(_height) {
		normal.normalize();
	}

	Capsule<3>& operator=(const Capsule<d>& copy) {center=copy.center;radius=copy.radius;normal=copy.normal;height=copy.height;return *this;}
	Capsule(const Capsule<d>& copy) {*this=copy;}
	...
	virtual real Phi(const VectorD& pos) const 
	{
		...
		if (fabs(diff_dot_normal) <= height / 2) {
			VectorD plane_diff = diff - diff_dot_normal * normal;
			return plane_diff.norm() - radius;
		}
		else {
			VectorD end_center = center;
			if (diff_dot_normal > 0) end_center += normal * height / 2;
			else end_center -= normal * height / 2;
			return (pos-end_center).norm();
		}
	}
	virtual VectorD Normal(const VectorD& pos) const
	{
...
////Cylinder
//////////////////////////////////////////////////////////////////////////
//// switched to Y axis (should specify this with an axis input)
template<int d> class Cylinder : public ImplicitGeometry<d>
{Typedef_VectorDii(d);					// normal not in use, perhaps someone knows SDF better can add support to normal
public:
	VectorD center = VectorD::Zero();
	real radius = (real).1;
	...
	Cylinder(const VectorD& _center, const real _radius, const VectorD& _normal, const real _height) 
		:center(_center), radius(_radius), normal(_normal), height(_height) 
	{
		normal.normalize();
	}

	Cylinder<3>& operator=(const Cylinder<d>& copy) {center=copy.center;radius=copy.radius;normal=copy.normal;height=copy.height;return *this;}
	Cylinder(const Cylinder<d>& copy) {*this=copy;}

	virtual bool Inside(const VectorD& pos) const {return Phi(pos)<=0;}
	virtual real Phi(const VectorD& pos) const 
	{
		VectorD diff = pos - center;
		Vector2 v = (Vector2(Vector2(diff[0], diff[2]).norm(), diff[1])).cwiseAbs() - Vector2(radius, height);
		return std::min(std::max(v[0], v[1]), 0.0) + v.cwiseMax(0.0).norm();
	}
	virtual VectorD Normal(const VectorD& pos) const
	{
		return VectorD::Unit(1);
	}
};
...
////InfiniteCylinder
//////////////////////////////////////////////////////////////////////////
template<int d> class InfiniteCylinder {};

template<> class InfiniteCylinder<3> : public ImplicitGeometry<3>
{
public:
	Vector3 c = Vector3::Zero();			//c[0]c[1] controls the center,c[2]is the radius

	InfiniteCylinder(const Vector3& _c) :c(_c){}

	InfiniteCylinder<3>& operator=(const InfiniteCylinder<3>& copy) { c = copy.c; return *this; }
	InfiniteCylinder(const InfiniteCylinder<3>& copy) { *this = copy; }

	virtual bool Inside(const Vector3& pos) const { return Phi(pos) <= 0; }
	virtual real Phi(const Vector3& pos) const
	{		//control this term to control the dir
		return (Vector2(pos[2],pos[1]) - Vector2(c[0],c[1])).norm() - c[2];
	}
	virtual Vector3 Normal(const Vector3& pos) const
	{
		return Vector3(c[0]-pos[2],c[1]-pos[1], 0).normalized();
	}
};
...
////Torus
//////////////////////////////////////////////////////////////////////////
template<int d> class Torus : public ImplicitGeometry<d>
{Typedef_VectorDii(d);
public:
	VectorD center=VectorD::Zero();
	Vector2d radius;
	...
	Torus<d>& operator=(const Torus<d>& copy) {center=copy.center;radius=copy.radius;return *this;}
	Torus(const Torus<d>& copy) {*this=copy;}
