...
////////////////////////////////////////////////////////////////////////////////////////////////////
////Initializer
template<int d> MacGrid<d>::MacGrid(){}
...
template<int d> MacGrid<d>& MacGrid<d>::operator=(const MacGrid& copy)
{
	grid=copy.grid;
	for(int i=0;i<d;i++){
		face_counts[i]=copy.face_counts[i];
		face_grids[i]=copy.face_grids[i];}
	return *this;
}

template<int d> void MacGrid<d>::Initialize_Helper()
{
	Initialize_Face_Counts();
	Initialize_Face_Grids();
	Initialize_Edge_Counts();
	Initialize_Edge_Grids();
}

template<int d> void MacGrid<d>::Initialize_Face_Counts()
{
	for(int i=0;i<d;i++){
		face_counts[i]=grid.cell_counts;face_counts[i][i]+=1;}
}

template<int d> void MacGrid<d>::Initialize_Face_Grids()
{
	for(int i=0;i<d;i++){
		VectorD domain_min=grid.domain_min+(VectorD::Ones()-VectorD::Unit(i))*grid.dx*(real).5;
		face_grids[i].Initialize(face_counts[i]-VectorDi::Ones(),grid.dx,domain_min);}
}

template<int d> void MacGrid<d>::Initialize_Edge_Counts()
{
	for(int i=0;i<d;i++){
		edge_counts[i]=grid.cell_counts+VectorDi::Ones();edge_counts[i][i]-=1;}
}

template<int d> void MacGrid<d>::Initialize_Edge_Grids()
{
	for(int i=0;i<d;i++){
		VectorD domain_min=grid.domain_min+(VectorD::Unit(i))* grid.dx* (real).5;
		edge_grids[i].Initialize(edge_counts[i]-VectorDi::Ones(),grid.dx,domain_min);}
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
////Edge operations
template<int d> bool MacGrid<d>::Valid_Edge(const int axis,const VectorDi& edge) const
{return edge_grids[axis].Valid_Node(edge);}

template<int d> bool MacGrid<d>::Is_Boundary_Edge(const int axis,const VectorDi& edge) const
{return edge_grids[axis].Is_Boundary_Node(edge);}

template<int d> bool MacGrid<d>::Is_Axial_Boundary_Edge(const int axis,const VectorDi& edge) const
{return edge[axis]==0||edge[axis]==edge_grids[axis].node_counts[axis]-1;}

template<int d> Vector<real,d> MacGrid<d>::Edge_Center(const int axis,const VectorDi& edge) const
{return edge_grids[axis].Node(edge);}

template<int d> int MacGrid<d>::Edge_Index(const int axis,const VectorDi& edge) const
{return edge_grids[axis].Node_Index(edge);}

template<int d> Vector<int,d> MacGrid<d>::Edge_Coord(const int axis,const int edge_index) const
{return edge_grids[axis].Node_Coord(edge_index);}

template<int d> int MacGrid<d>::Number_Of_Edges(const int axis) const
{return edge_grids[axis].node_counts.prod();}

template<int d> int MacGrid<d>::Number_Of_Edges() const
{int n=0;for(int axis=0;axis<d;axis++)n+=edge_grids[axis].node_counts.prod();return n;}

template<int d> Vector<int,d> MacGrid<d>::Edge_Incident_Cell(const int axis,const VectorDi& edge,const int i) {/*not impl*/return VectorDi::Zero();}
template<> Vector<int,2> MacGrid<2>::Edge_Incident_Cell(const int axis,const Vector2i& edge,const int i)
{static const Vector2i cell_offset[4]={Vector2i(0,-1),Vector2i(0,0),Vector2i(-1,0),Vector2i(0,0)};
assert(i>=0&&i<2);return edge+cell_offset[axis*2+i];}
template<> Vector<int,3> MacGrid<3>::Edge_Incident_Cell(const int axis,const Vector3i& edge,const int i)
{static const Vector3i node_offset[12]={Vector3i(0,-1,-1),Vector3i(0,-1,0),Vector3i(0,0,-1),Vector3i(0,0,0),
Vector3i(-1,0,-1),Vector3i(-1,0,0),Vector3i(0,0,-1),Vector3i(0,0,0),Vector3i(-1,-1,0),Vector3i(-1,0,0),Vector3i(0,-1,0),Vector3i(0,0,0)};
assert(i>=0&&i<4);return edge+node_offset[axis*4+i];}

template<int d> Vector<int,d> MacGrid<d>::Edge_Incident_Node(const int axis,const VectorDi& edge,const int i) {assert(i>=0&&i<2);return edge+VectorDi::Unit(axis)*i;}

template<int d> Vector<int,d> MacGrid<d>::Cell_Incident_Edge(const int axis,const VectorDi& cell,const int i) {/*not impl*/return VectorDi::Zero();}
template<> Vector<int,2> MacGrid<2>::Cell_Incident_Edge(const int axis,const Vector2i& cell,const int i)
{static const Vector2i cell_offset[4]={Vector2i(0,0),Vector2i(0,1),Vector2i(0,0),Vector2i(1,0)};
assert(i>=0&&i<2);return cell+cell_offset[axis*2+i];}
template<> Vector<int,3> MacGrid<3>::Cell_Incident_Edge(const int axis,const Vector3i& cell,const int i)
{static const Vector3i node_offset[12]={Vector3i(0,0,0),Vector3i(0,0,1),Vector3i(0,1,0),Vector3i(0,1,1),
Vector3i(0,0,0),Vector3i(0,0,1),Vector3i(1,0,0),Vector3i(1,0,1),Vector3i(0,0,0),Vector3i(0,1,0),Vector3i(1,0,0),Vector3i(1,1,0)};
assert(i>=0&&i<4);return cell+node_offset[axis*4+i];}
