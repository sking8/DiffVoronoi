...
namespace IntpFunc
{
		...
		std::function<real(const Vector2&)> Phi,const Vector2& frac)
	{
		real w[4], phi[4], f[4];
		for (int s = 0;s < 4;s++) {
			//dx,dy,dz iterating in {0,0,0}~{1,1,1}
			int dx = s & 1;
			int dy = (s >> 1) & 1;
			w[s] = (frac[0] * dx + (1 - frac[0]) * (1 - dx))
				* (frac[1] * dy + (1 - frac[1]) * (1 - dy));
			Vector2i celli(cell[0] + dx, cell[1] + dy);
			f[s] = field(celli);
			phi[s] = Phi(grid.Node(celli));
		}
		...
		return (w[0] * f[0] + w[1] * f[1] + w[2] * f[2] + w[3] * f[3]) / total_w;
	}
		...
		std::function<real(const Vector3&)> Phi,const Vector3& frac)
	{
		...
		for (int s = 0;s < 8;s++) {
			...
			w[s] = (frac[0] * dx + (1 - frac[0]) * (1 - dx))
				* (frac[1] * dy + (1 - frac[1]) * (1 - dy))
				* (frac[2] * dz + (1 - frac[2]) * (1 - dz));
			Vector3i celli(cell[0] + dx, cell[1] + dy, cell[2] + dz);
			f[s] = field(celli);
			phi[s] = Phi(grid.Node(celli));
		}
		...
		return (w[0] * f[0] + w[1] * f[1] + w[2] * f[2] + w[3] * f[3] + w[4] * f[4] + w[5] * f[5] + w[6] * f[6] + w[7] * f[7]) / total_w;
	}
...
public:
	...
	template<class T> T Interpolate_Centers_With_Phi(const Field<T,d>& field,std::function<real(const VectorD&)> Phi,const VectorD& pos) const \
	{
		...
		return IntpFunc::Nodes_To_Point_Linear_With_Phi(field,cell,center_grid,Phi,frac);
	}
