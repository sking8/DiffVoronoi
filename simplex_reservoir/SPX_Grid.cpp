...
{
    cell_counts=copy.cell_counts;
    node_counts=copy.node_counts;
    dx=copy.dx;
    domain_min=copy.domain_min;
    domain_max=copy.domain_max;
    for(int i=0;i<d*2;i++)cell_nb_c_idx_offset[i]=copy.cell_nb_c_idx_offset[i];
    for(int i=0;i<d*2;i++)node_nb_c_idx_offset[i]=copy.node_nb_c_idx_offset[i];
    return *this;
}
...
template<int d> void Grid<d>::Initialize(const VectorDi& _cell_counts,const real _dx,const VectorD& _domain_min)
{
    ...
    node_counts=cell_counts+VectorDi::Ones();
    dx=_dx;
    domain_min=_domain_min;
    domain_max=domain_min+cell_counts.template cast<real>()*dx;
...
template<int d> Vector<int,d> Grid<d>::Cell_Coord(const VectorD& pos) const 
{VectorD coord_with_frac=(pos-domain_min)/dx;return coord_with_frac.template cast<int>();}
...
template<> Vector<int,1> Grid<1>::Nb_C(const Vector1i& coord,const int i)//-x,+x
{static int neighbor_offset[2]={-1,1};return coord+Vec1i(neighbor_offset[i]);}
template<> Vector<int,2> Grid<2>::Nb_C(const Vector2i& coord,const int i)//-x,-y,+y,+x
...
template<> Vector<int,3> Grid<3>::Nb_C(const Vector3i& coord,const int i)//-x,-y,-z,+z,+y,+x
...
template<int d> int Grid<d>::Nb_C_Side(const int i){return 0;}
template<> int Grid<1>::Nb_C_Side(const int i){static int sides[2]={0,1};return sides[i];}
template<> int Grid<2>::Nb_C_Side(const int i){static int sides[4]={0,0,1,1};return sides[i];}
template<> int Grid<3>::Nb_C_Side(const int i){static int sides[6]={0,0,0,1,1,1};return sides[i];}
