...
template<int d> void SoftBodyLinearFemHex<d>::Update_K_And_f()
{
	...
	for(int i=0;i<(int)mesh->elements.size();i++){
		...
		LinearFemFunc::Hex_Stiffness_Matrix<d>(materials[mat].youngs_modulus,materials[mat].poisson_ratio,hex,Ke);
		LinearFemFunc::Add_Cell_Stiffness_Matrix<d>(K,Ke,idx);
	}
