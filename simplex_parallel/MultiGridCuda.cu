...
template<class MtxT, class T> int Preconditioned_Conjugate_Gradient_Device(MtxT* A, T* b, T* x, T* r, T* d, T* q, T* s, T* pd, int iter_num, T tolerance, bool use_precond/*=true*/)
{
	...
	if (verbose) Info("CUDA PCG converges in {} to abs error {}", iter, delta);
	//if(verbose)std::cout<<"cuda pcg converges in "<<iter<<" iterations"<<std::endl;
	return iter;
}

template<class MtxT, class T> int Preconditioned_Conjugate_Gradient(MtxT* A_host, T* _b, T* _x, int iter_num, T tolerance, bool use_precond)
{
	if(!Is_Cuda_Context_Initialized())Initialize_Cuda_Context();
	...
	int iters = Preconditioned_Conjugate_Gradient_Device(A, b, x, r, d, q, s, pd, iter_num, tolerance, use_precond);
...
template int Preconditioned_Conjugate_Gradient<SparseMatrixCuda<T>,T>(SparseMatrixCuda<T>* A_host,T* _b,T* _x,int iter_num,T tolerance,bool use_precond); \
template int Preconditioned_Conjugate_Gradient_Device<SparseMatrixCuda<T>,T>(SparseMatrixCuda<T>* A_dev,T* b,T* x,T* r,T* d,T* q,T* s,T* pd,int iter_num,T tolerance,bool use_precond); \
template int Preconditioned_Conjugate_Gradient<SparseMatrix<T>,T>(SparseMatrix<T>* A_host,T* _b,T* _x,int iter_num,T tolerance,bool use_precond); \
