...
namespace AuxFunc{
	...
	template<class T> T Lerp(const T a, const T b, const real t) { return a * t + b * (1 - t); }
	...
	template<class T> T Abs_Mean(const Array<T>& v) { T c = Zero<T>();for (auto i = 0;i < v.size();i++)c += abs(v[i]);c /= (real)v.size();return c; }
